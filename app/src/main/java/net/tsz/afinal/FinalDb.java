...
public class FinalDb {
     ...
     *@Description: 分页查询
     *@Author: 李焱生
     *@Since: 2015年4月9日下午10:49:00
     *@param clazz
     *@param strWhere
     *@param orderBy--排序字段
     *@param orderByType--排序类型 0--顺序 1-－倒序
     *@param pageNo
     *@param pageSize
     *@return
     */
    public <T> List<T> findAllByWhere(Class<T> clazz,String strWhere,String orderBy,int orderByType,int pageNo,int pageSize){
        checkTableExist (clazz);
        int firstResult = (pageNo - 1) * pageSize;
        StringBuffer sb = new StringBuffer ();
        sb.append (SqlBuilder.getSelectSQLByWhere (clazz, strWhere));
        if(orderByType == 1)
        {
        ...
        return findAllBySql (clazz, sb.toString ());
    }
